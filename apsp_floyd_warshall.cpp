...
int main()
{
	...
	cin.tie(NULL);
		...
		freopen("apsp_floyd_warshall_in.txt","r",stdin);
...
//--------------------------------------------------------------------------------------
	...
	re(i,n)
		re(j,n)
			if(i!=j)
				dist[i+1][j+1]=1000000000;
			else
				dist[i+1][j+1]=0;

	re(i,m){
		cin>>a>>b>>w;
		// dist[a][b]=min(w,dist[a][b]);  if multiple edges exist between 2 nodes
		dist[a][b]=w;
	}
